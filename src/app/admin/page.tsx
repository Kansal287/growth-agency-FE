...
import { AlertCircle, LogIn, Lock, User, Shield } from "lucide-react";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response?.data?.success) {
        setAdminToken(response?.data?.data?.access_token);
        router.replace("/admin/dashboard");
      }
    } catch (err: any) {
      ...
      <div className="w-full max-w-md bg-[#0f131a]/80 backdrop-blur-xl border border-slate-800/80 rounded-3xl shadow-2xl p-6 sm:p-8 space-y-6 transition-all duration-300 hover:shadow-indigo-500/5 relative z-10">
        ...
        <form onSubmit={handleSubmit} className="space-y-4" noValidate>
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full h-11 bg-linear-to-r from-indigo-600 via-violet-600 to-blue-600 hover:opacity-95 disabled:opacity-50 text-white font-bold rounded-xl text-xs flex items-center justify-center gap-2 cursor-pointer shadow-lg shadow-indigo-500/20 active:scale-[0.98] transition-all mt-6"
          >
            {loading ? (
              <>
